...
parser.add_argument('--glow', action='store_true', help='Add glow effect to text')
...
def random_rainbow():
    ...
    r, g, b = colorsys.hsv_to_rgb(h, 1, 1)  # Full saturation and value
...
moon_size = 100
if args.rainbow:
    # Simple rainbow gradient for moon: Draw multiple thin arcs
    for i in range(10):  # 10 layers for gradient
        color = random_rainbow()
        draw.arc((moon_x, moon_y, moon_x + moon_size, moon_y + moon_size), start=90, end=270, fill=color, width=1)
    # Inner cut remains bg color
    draw.arc((moon_x + 20, moon_y, moon_x + moon_size + 20, moon_y + moon_size), start=90, end=270, fill=bg_rgb, width=10)
else:
    draw.arc((moon_x, moon_y, moon_x + moon_size, moon_y + moon_size), start=90, end=270, fill=star_rgb, width=10)
    draw.arc((moon_x + 20, moon_y, moon_x + moon_size + 20, moon_y + moon_size), start=90, end=270, fill=bg_rgb, width=10)
...
for _ in range(args.num_stars):
    ...
    if args.rainbow:
        ...
        color_variation = (max(0, star_rgb[0] + random.randint(-20, 20)), 
...
text_y = (args.image_size - total_height) / 2  # Start y for first line

if args.glow:
    # Apply glow to each line
    ...
    draw = ImageDraw.Draw(img)  # Redraw on blurred image
...
while os.path.exists(output_path):
    output_path = f"{base_name}_{counter:02d}{ext}"
    counter += 1
